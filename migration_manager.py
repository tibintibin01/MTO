...
def get_applied_versions():
    ...
    try:
        ...
        exists = db.db_query("SHOW TABLES LIKE 'schema_versions'", fetch=True, commit=False)
        if not exists:
            return set()
        
        rows = db.db_query("SELECT version FROM schema_versions", fetch=True, commit=False)
