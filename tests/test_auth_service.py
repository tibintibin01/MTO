...
def test_create_user_mocked(mock_db_session, monkeypatch):
    ...
    assert audit_call["action"] == "CREATE_USER"
    assert audit_call["username"] == "superuser"
    assert audit_call["db_session"] is mock_db_session
