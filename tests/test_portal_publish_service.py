...
def test_generate_portal_snapshot_is_sanitized_and_checksummed(db, monkeypatch):
    ...
    assert snapshot["checksum"] == _snapshot_checksum(
    ...
    assert (
        ...
        in snapshot["owner_lookup_index"]
    )
    assert snapshot["owner_lookup_index"][
        _owner_lookup_hash("JUAN", "test-lookup-secret")
    ] == [0]
...
def test_save_portal_snapshot_uses_configured_directory_and_atomic_files(
    ...
    assert result["latest_gzip_path"] == str(
        tmp_path / "portal_snapshot_latest.json.gz"
    )
    assert (
        json.loads(
            (tmp_path / "portal_snapshot_latest.json").read_text(encoding="utf-8")
        )
        == snapshot
    )
...
def test_snapshot_uses_current_year_assessment_and_labels_future_revaluation(
    db, monkeypatch
):
