...
class ModernChartWidget:
    def __init__(self, parent, title):
        ...
        if self.matplotlib:
            ...
            ctk.CTkLabel(self.card, text="Charts unavailable (matplotlib missing)").pack(expand=True)

    def pack(self, **kwargs):
        self.card.grid(**kwargs) if "row" in kwargs else self.card.pack(**kwargs)

    def draw(self, x_data, y_data, chart_type="bar", color="#1f538d", no_data_msg="No data available"):
        if not self.ax or not self.matplotlib: return
...
class ToastNotification(ctk.CTkToplevel):
    def __init__(self, master, message, color="#2ecc71", duration=3000):
        ...
        self.label = ctk.CTkLabel(self, text=message, font=ModernTheme.BODY, text_color="white", wraplength=280)
