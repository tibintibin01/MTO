...
class DashboardHomePage:
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(header, text="Revenue Dashboard", font=ModernTheme.H1, text_color="white").pack(anchor="w", padx=30, pady=(25, 5))
        ctk.CTkLabel(header, text="Real-time collection monitoring and analytics", font=ModernTheme.BODY, text_color="#d1d1d1").pack(anchor="w", padx=30, pady=(0, 25))
        ...
        self.stat_cards = {
            "total_properties": self._make_stat_card(stats_frame, 0, "Total Properties", "0", "#3498db"),
            "collections_today": self._make_stat_card(stats_frame, 1, "Today's Collection", "P 0.00", "#2ecc71"),
            "collections_month": self._make_stat_card(stats_frame, 2, "Monthly Total", "P 0.00", "#e67e22")
        }
        ...
        ctk.CTkLabel(self.backup_card, text="SYSTEM DATA PROTECTION STATUS", font=("Segoe UI", 12, "bold"), text_color="gray").pack(pady=(15, 10), padx=20, anchor="w")
        ...
        self.backup_labels = {
            ...
            "verify": self._make_backup_item(inner_backup, 3, "Restore Test", "Unknown")
        }
        
        self.backup_btn = ctk.CTkButton(self.backup_card, text="RUN HYBRID BACKUP NOW", command=self.trigger_manual_backup,
                                        width=200, height=35, font=ModernTheme.BUTTON)
    ...
    def trigger_manual_backup(self):
        try:
            ...
            show_toast(self.parent.winfo_toplevel(), "Hybrid backup started in background", type="info")
...
class DashboardApp(ctk.CTk):
    ...
    def setup_sidebar(self):
        ...
        if auth.has_permission(self.user_data, "property_view"):
            self.create_nav_btn("Property Records", lambda: self.load_page(PropertyPage))
        if auth.has_permission(self.user_data, "ledger_view"):
            self.create_nav_btn("Payment & Receipt Ledger", lambda: self.load_page(LedgerPage))
        
        ctk.CTkLabel(self.nav_scroll, text="ADVANCED", font=("Segoe UI", 10, "bold"), text_color="gray").pack(pady=(20, 5))
        
        if auth.has_permission(self.user_data, "report_view"):
            self.create_nav_btn("Reports", lambda: self.load_page(ReportsPage))
        
        self.create_nav_btn("Assessment Roll", lambda: self.load_page(AssessmentRollPage))
    ...
    def create_nav_btn(self, text, command):
        btn = ctk.CTkButton(self.nav_scroll, text=text, command=command, anchor="w", 
                            fg_color="transparent", text_color=("gray10", "gray90"),
                            hover_color=("gray70", "gray30"), font=ModernTheme.BODY, height=45)
...
class SystemHelpPage:
    def __init__(self, parent, user):
        ...
        ctk.CTkLabel(self.container, text="MTO Treasury System Help Guide", 
                     font=ModernTheme.H1, text_color="#3498db").pack(anchor="w", pady=(0, 20))
        ...
        help_text = [
            ...
            ("⌨️ Keyboard Shortcuts", "Ctrl + F: Quick Search / Command Palette\nCtrl + P: Open Command Palette\nCtrl + E: Export visible table data to Excel/CSV"),
        ...
        for title, desc in help_text:
            ...
            ctk.CTkLabel(f, text=title, font=ModernTheme.H3, text_color="#3498db").pack(anchor="w", padx=15, pady=(10, 2))
            ctk.CTkLabel(f, text=desc, font=ModernTheme.BODY, text_color=("#333333", "#cccccc"),
                         wraplength=800, justify="left").pack(anchor="w", padx=20, pady=(0, 15))
