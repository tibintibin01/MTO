...
class ConnectivityStatusBar(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.status_dot = ctk.CTkLabel(
            self, text="●", font=("Segoe UI", 14), text_color="#2ecc71"
        )
        ...
        self.queue_lbl = ctk.CTkLabel(
            self, text="", font=("Segoe UI", 10), text_color="#bdc3c7"
        )
    ...
    def update_status(self):
        ...
        quarantined = manager.get_quarantined_count()
        ...
        if status == "ONLINE":
            ...
            self.status_lbl.configure(text="OFFLINE - READ-ONLY CACHE ONLY")
        elif status == "SYNCING":
        ...
        if quarantined > 0:
            self.queue_lbl.configure(
                text=f"REVIEW REQUIRED: {quarantined} BLOCKED LEGACY ITEM"
