...
class AssessmentsPage:
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.container, text="ASSESSMENTS AND TAX ROLL", font=ModernTheme.H2).pack(anchor="w", pady=(0, 20))
    ...
    def setup_tax_roll_tab(self):
        ...
        ctk.CTkButton(filter_fr, text="LOAD ASSESSMENT ROLL", command=self.load_assessment_roll).pack(side="left", padx=10)

        self.roll_tree_fr = ctk.CTkFrame(self.tax_roll_tab)
        self.roll_tree_fr.pack(fill="both", expand=True, padx=10, pady=10)
        
        cols = ("TD Number", "Owner Name", "Location", "Kind", "Assessed Value", "Basic Tax", "SEF Tax")
    ...
    def setup_delinquency_tab(self):
        ...
        ctk.CTkButton(btn_fr, text="FETCH DELINQUENT ACCOUNTS", command=self.load_delinquent_accounts).pack(side="left", padx=10)

        cols = ("TD Number", "Owner Name", "Location", "Assessed Value", "Years Delinq", "Total Due")
