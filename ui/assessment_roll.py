...
import services.api_helper as api
...
class AssessmentRollPage:
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(header, text="ASSESSMENT ROLL", font=ModernTheme.H2).pack(side="left")
        ...
        # Search Box
        self.search_ent = ctk.CTkEntry(filters_fr, placeholder_text="Search PIN, TD, or Owner...", width=250)
        self.search_ent.pack(side="left")
        self.search_ent.bind("<Return>", lambda e: self.refresh_table())
        
        # Kind of Property Filter
        ctk.CTkLabel(filters_fr, text="KIND:", font=("Segoe UI", 10, "bold"), text_color="gray").pack(side="left", padx=(20, 5))
        self.kind_var = tk.StringVar(value="ALL")
        self.kind_cb = ctk.CTkComboBox(filters_fr, values=["ALL", "LAND", "BUILDING", "MACHINERY", "OTHERS"], variable=self.kind_var, width=140)
        self.kind_cb.pack(side="left")
        self.kind_cb.configure(command=lambda e: self.refresh_table())
        
        # Year Range
        ctk.CTkLabel(filters_fr, text="YEAR FROM:", font=("Segoe UI", 10, "bold"), text_color="gray").pack(side="left", padx=(20, 5))
        self.year_start_ent = ctk.CTkEntry(filters_fr, width=80, placeholder_text="YYYY")
        self.year_start_ent.pack(side="left")
        
        ctk.CTkLabel(filters_fr, text="TO:", font=("Segoe UI", 10, "bold"), text_color="gray").pack(side="left", padx=(5, 5))
        self.year_end_ent = ctk.CTkEntry(filters_fr, width=80, placeholder_text="YYYY")
        self.year_end_ent.pack(side="left")
        
        ctk.CTkButton(filters_fr, text="🔍 REFRESH", command=self.refresh_table, width=100, fg_color="#34495e").pack(side="left", padx=10)
        
        ctk.CTkButton(filters_fr, text="📥 IMPORT EXCEL", command=self.import_excel, fg_color="#3498db", width=120).pack(side="right", padx=10)
        ctk.CTkButton(header, text="+ ADD RECORD", command=self.open_modal, fg_color="#27ae60", width=120).pack(side="right")
        ...
        style.configure("Roll.Treeview", rowheight=35, font=("Segoe UI", 10), background="#2b2b2b", fieldbackground="#2b2b2b", foreground="white")
        style.configure("Roll.Treeview.Heading", font=("Segoe UI", 10, "bold"), background="#333333", foreground="white")
        
        self.cols = ("ID", "TD NO.", "PIN", "LOT & BLK", "PROPERTY OWNER", "LOCATION", "CLASSIFICATION", "ASSESSED VALUE", "PREVIOUS TD", "EFFECTIVITY")
        self.tree = ttk.Treeview(table_fr, columns=self.cols, show="headings", style="Roll.Treeview")
        
        # Column Config
        col_widths = {
            "ID": 0, "TD NO.": 110, "PIN": 130, "LOT & BLK": 100, 
            "PROPERTY OWNER": 200, "LOCATION": 130, "CLASSIFICATION": 120,
            "ASSESSED VALUE": 120, "PREVIOUS TD": 110, "EFFECTIVITY": 100
        }
        ...
        def worker():
            try:
                ...
                results = prop_svc.search_properties(
                    ...
                    kind=kind, year_start=y_start, year_end=y_end
                )
    ...
    def _update_table(self, results):
        ...
        if not results and self.search_ent.get().strip():
            messagebox.showinfo("Assessment Roll", "Record not found. This property may not be registered in the Assessment Roll yet.")
            return
        ...
        for i, r in enumerate(results):
            ...
            self.tree.insert("", "end", values=(r[0], td, pin, lot_blk, owner, loc, kind, av, prev, eff), tags=(tag,))
        
    def import_excel(self):
        file_path = filedialog.askopenfilename(title="Select Assessment Roll Excel", filetypes=[("Excel files", "*.xlsx *.xls")])
        ...
        ctk.CTkLabel(msg_win, text="⚙️ PROCESSING BULK IMPORT...", font=("Segoe UI", 12, "bold")).pack(expand=True)
        msg_win.update()

        def worker():
            try:
                # Use centralized API helper
                res = api.api_request("POST", "/properties/import-assessment", files={"file": open(file_path, "rb")})
                
                self.container.after(0, lambda: [msg_win.destroy(), self._show_import_summary(res)])
            except Exception as e:
                self.container.after(0, lambda e=e: [msg_win.destroy(), messagebox.showerror("Import Error", str(e))])
...
class AssessmentModal(ctk.CTkToplevel):
    ...
    def setup_ui(self):
        ...
        # Header
        ctk.CTkLabel(self, text="📜 NEW ASSESSMENT RECORD", font=("Segoe UI", 16, "bold"), text_color="#1f538d").pack(pady=20)
        ...
        for label, key in fields:
            ctk.CTkLabel(container, text=label.upper(), font=("Segoe UI", 9, "bold"), text_color="gray").pack(anchor="w", padx=15, pady=(10, 0))
            var = tk.StringVar()
        ...
        # Barangay Dropdown
        ctk.CTkLabel(container, text="BARANGAY *", font=("Segoe UI", 9, "bold"), text_color="gray").pack(anchor="w", padx=15, pady=(10, 0))
        self.brgy_var = ctk.StringVar(value="SELECT BARANGAY")
        self.brgy_drop = ctk.CTkComboBox(container, values=[
            "NORTH POBLACION", "SOUTH POBLACION", "BAYABAS", "BORLONGAN", 
            "BUENAVISTA", "CALAOCAN", "DIAMANEN", "DIANED", "DIARABASIN", 
            "DIBUTUNAN", "DIMABUNO", "DINADIAWAN", "DITALE", "GUPA", "IPIL", 
            "LABOY", "LIPIT", "LOBBOT", "MALIGAYA", "MIJARES", "MUCDOL", 
            "PUANGI", "SALAY", "SAPANGKAWAYAN", "TOYTOYAN"
        ], variable=self.brgy_var, height=38)
        self.brgy_drop.pack(fill="x", padx=15, pady=(0, 20))

        # Footer
        footer = ctk.CTkFrame(self, fg_color="white", height=80, border_width=1, border_color="#e0e0e0")
        footer.pack(side="bottom", fill="x")
        
        ctk.CTkButton(footer, text="CANCEL", command=self.destroy, fg_color="#95a5a6", width=120, height=45).pack(side="left", padx=30, pady=15)
        ctk.CTkButton(footer, text="SAVE TO ROLL", command=self.save, fg_color="#2ecc71", width=250, height=45).pack(side="right", padx=30, pady=15)
    ...
    def save(self):
        ...
        if not data["td_number"] or not data["owner_name"] or data["barangay"] == "SELECT BARANGAY":
            messagebox.showerror("Error", "TD Number, Owner, and Barangay are required!")
            return
        ...
        try:
            ...
            messagebox.showinfo("Success", "Assessment Record successfully added to the Roll.", parent=self)
            self.callback()
            self.destroy()
        except Exception as e:
            self.attributes("-topmost", False)
