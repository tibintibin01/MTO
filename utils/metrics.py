...
class MetricsManager:
    ...
    def record_request(method: str, endpoint: str, status: int, duration: float):
        ...
        REQUEST_COUNT.labels(
            method=method, endpoint=endpoint_label, status=status
        ).inc()
        REQUEST_LATENCY.labels(method=method, endpoint=endpoint_label).observe(duration)
...
_UUID_OR_TOKEN = re.compile(
    r"^(?:[0-9a-f]{8}-[0-9a-f-]{27,}|[0-9]{2,}|[A-Za-z0-9_-]{32,})$",
    re.IGNORECASE,
)


def normalize_endpoint_label(endpoint: str) -> str:
    """Bound Prometheus cardinality when a raw path reaches the metrics API."""
    ...
    for part in path.split("/"):
        ...
        elif _UUID_OR_TOKEN.fullmatch(part):
            parts.append("{id}")
        else:
            parts.append(part[:64])
    normalized = "/" + "/".join(parts)
    return normalized[:200] if normalized != "/" else "/"
