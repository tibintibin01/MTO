...
def verify_sql_dump(sql_file_path):
    ...
    try:
        ...
        if db_pass:
            create_cmd.insert(2, f"-p{db_pass}")

        subprocess.run(
            create_cmd,
            input=f"CREATE DATABASE IF NOT EXISTS {test_db_name};",
            text=True,
            check=True,
            timeout=180,
        )
        ...
        if db_pass:
            import_cmd.insert(2, f"-p{db_pass}")
        ...
        if db_pass:
            verify_cmd.insert(2, f"-p{db_pass}")

        result = subprocess.run(
            verify_cmd, capture_output=True, text=True, check=True, timeout=180
        )
        ...
        subprocess.run(
            create_cmd,
            input=f"DROP DATABASE {test_db_name};",
            text=True,
            check=True,
            timeout=180,
        )
        ...
        return (
            False,
            f"Verification failed: {e.stderr if hasattr(e, 'stderr') else str(e)}",
        )
