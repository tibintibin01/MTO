...
class BankDepositCreateSchema(BaseModel):
    ...
    bank_name: str = Field(
        ..., min_length=2, max_length=255, description="Name of the bank"
    )
    reference_number: str = Field(
        ..., min_length=2, max_length=255, description="Deposit slip reference number"
    )
...
@router.post(
    "/deposits",
    response_model=BankDepositResponseSchema,
...
async def log_bank_deposit(
    ...
    try:
        ...
        return BankDepositResponseSchema(
            ...
            created_at=(
                new_deposit.created_at.isoformat()
                if new_deposit.created_at
...
async def list_bank_deposits(
    ...
    deposits = (
        ...
        .filter(
            BankDeposit.date_deposited >= start_dt, BankDeposit.date_deposited <= end_dt
        )
        .order_by(BankDeposit.date_deposited.desc())
        .all()
    )
    ...
    return [
        BankDepositResponseSchema(
            ...
            created_at=(
                d.created_at.isoformat() if d.created_at else datetime.now().isoformat()
            ),
        )
        for d in deposits
    ]
...
async def export_coa_rcd(
    start_date: str = Query(..., description="Start date (YYYY-MM-DD)"),
    end_date: str = Query(..., description="End date (YYYY-MM-DD)"),
    liquidating_officer: str = Query(
        "N/A", description="Liquidating Officer Signatory"
    ),
