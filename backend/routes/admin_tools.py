...
async def td_number_audit(
    ...
    pay_rows = (
        ...
        .order_by(
            Payment.property_id.asc(), Payment.or_number.asc(), Payment.tax_year.asc()
        )
        .all()
    )
    ...
    # ── 4. Shadow duplicates ──────────────────────────────────────────────────
    valid_td_set = {
        (row[1] or "").strip() for row in rows if PATTERN.match((row[1] or "").strip())
    }
    ...
    td_lookup = {(row[1] or "").strip(): (row[0], row[2] or "") for row in rows}
    shadow_duplicates = []
    for row in rows:
        prop_id, td, owner = row[:3]
        td_str = (td or "").strip()
    ...
    audit_chain = verify_audit_chain(db_session)
    return {
...
async def td_number_fix(
    ...
    already_valid = 0
    td_to_prop_id = {
        (p.td_number or "").strip(): p.id for p in rows if (p.td_number or "").strip()
    }
    ...
    existing_tds = {
        r[0]
        for r in db_session.query(Property.td_number)
        .filter(Property.deleted_at == None)
        .all()
    }
    ...
    for item in fixed_list:
        ...
        if item["fixed"] in check_set:
            collision_list.append(
                {**item, "reason": f"Collision: '{item['fixed']}' already exists"}
            )
        else:
    ...
    db_session.commit()
    mto_logger.info(
        f"TD number auto-fix: {applied} fixed, {len(unfixable_list)} unfixable, {len(collision_list)} collisions",
...
async def shadow_duplicate_cleanup(
    ...
    for pid in bad_ids:
        ...
        prop.deleted_at = now
        deleted.append(
            {"id": pid, "td_number": prop.td_number, "owner_name": prop.owner_name}
        )
            ...
            sample.append(
                {
                    "property_id": prop.id,
                    "td_number": prop.td_number,
                    "before": changes.get("owner_name", {}).get(
                        "before", prop.owner_name
                    ),
                    "after": changes.get("owner_name", {}).get(
                        "after", prop.owner_name
                    ),
...
async def update_tax_policy(
    ...
    for field, val in [
        ...
        ("sef_rate", data.sef_rate),
        ("penalty_rate", data.penalty_rate),
    ]:
        if not (0 <= val <= 0.10):
            raise HTTPException(
                status_code=400, detail=f"{field} must be between 0 and 10%."
            )
    ...
    db_session.commit()
    mto_logger.info(
        f"Tax policy updated for {tax_year}", user=current_user.get("username")
    )
...
async def sync_billing_years(
    ...
    mto_logger.info(
        f"Billing year sync requested (dry_run={dry_run})",
        user=current_user.get("username"),
    )
