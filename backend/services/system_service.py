...
def restore_database(sql_file_path):
    ...
    safety_backup = os.path.join(backups_dir, f"PRE_RESTORE_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.sql")
    if not backup_database(safety_backup):
        raise RuntimeError("Could not create the automatic safety backup. Restore cancelled.")
...
def get_report_summary(selected_month="All", selected_year="All"):
    ...
    if not rows: return {"total_amount": 0.0, "payment_count": 0, "property_count": 0, "latest_payment": ""}
    row = rows[0]
    return {
    ...
    return [
        {
            ...
            "table_name": r[4], "record_id": r[5], "old_values": r[6], "new_values": r[7], "ip_address": r[8]
        }
        for r in rows
    ]
...
def delete_old_audit_logs(days=365):
    def operation(cur):
        cur.execute(f"DELETE FROM audit_logs WHERE timestamp < NOW() - INTERVAL %s DAY", (int(days),))
        return cur.rowcount
