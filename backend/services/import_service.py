...
class DataCleanser:
    ...
    def normalize_barangay(val, known_barangays=None):
        ...
        if not known_barangays:
            return val
        # Basic fuzzy match or exact match
        if val in known_barangays:
            return val
        # Check for abbreviations
        if val == "N. POBLACION":
            return "NORTH POBLACION"
        if val == "S. POBLACION":
            return "SOUTH POBLACION"
        return val
...
def validate_property_import(file_content, file_extension, db_session: Session = None):
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            owner = (
                DataCleanser.to_str(row_data.get(owner_col, "")) if owner_col else ""
            )
                ...
                rows_to_import.append(
                    {
                        ...
                        "penalty": (
                            DataCleanser.to_float(row_data.get(found_cols["penalty"]))
                            if found_cols.get("penalty")
                            else 0.0
                        ),
                        "discount": (
                            DataCleanser.to_float(row_data.get(found_cols["discount"]))
                            if found_cols.get("discount")
                            else 0.0
                        ),
                        "amount_paid": (
                            DataCleanser.to_float(
                                row_data.get(found_cols["amount_paid"])
                            )
                            if found_cols.get("amount_paid")
                            else 0.0
                        ),
                    }
                )
...
def validate_assessment_import(
    file_content, file_extension, db_session: Session = None
):
    """
        ...
        for index, row in df.iterrows():
            ...
            results.append(
                {
                    ...
                    "td_number": td,
                    "owner_name": owner,
                    "lot_number": DataCleanser.to_str(
                        row.get(found_cols.get("lot_number"))
                    ),
                    "status": status,
            ...
            if not errors:
                ...
                raw_row = {
                    ...
                    "owner_name": owner,
                    "assessed_value": val,
                    "location": DataCleanser.to_str(
                        row.get(found_cols.get("location"))
                    ),
                    "pin": DataCleanser.to_str(row.get(found_cols.get("pin"))),
                    "tax_year": DataCleanser.to_str(
                        row.get(found_cols.get("tax_year"))
                    ),
                    "area": DataCleanser.to_str(row.get(found_cols.get("area"))),
                    "kind_of_property": DataCleanser.to_str(
                        row.get(found_cols.get("kind"), "LAND")
                    ).upper(),
                    "lot_number": DataCleanser.to_str(
                        row.get(found_cols.get("lot_number"))
                    ),
                    "block_number": DataCleanser.to_str(
                        row.get(found_cols.get("block_number"))
                    ),
                }
...
def commit_assessment_import(data_list, user, db_session: Session = None):
    ...
    try:
                ...
                if i % 10 == 0 or i == total - 1:
                    ...
                    try:
                        asyncio.run_coroutine_threadsafe(
                            report_progress(
                                percentage, f"Importing: {i+1} / {total} records"
                            ),
                            loop,
                        )
                    except Exception as prog_err:
                        mto_logger.warning(
                            f"Failed to submit progress update: {prog_err}"
                        )
        ...
        log_action(
            user,
            f"Wizard Assessment Import: {inserted} new, {updated} updated, {failed} failed.",
            db_session=db_session,
        )
        db_session.commit()
        return {
...
def commit_property_import(data_list, user, db_session: Session = None):
    ...
    try:
        for i, row in enumerate(data_list):
            ...
            if i % 10 == 0 or i == total - 1:
                ...
                try:
                    asyncio.run_coroutine_threadsafe(
                        report_progress(
                            percentage, f"Processing: {i+1} / {total} properties"
                        ),
                        loop,
                    )
                except Exception as e:
        ...
        log_action(
            user, f"Bulk imported {count} property records.", db_session=db_session
        )
        db_session.commit()
        return count
    except Exception as e:
...
def import_assessment_roll_from_excel(file_path, user, db_session: Session = None):
    ...
    try:
        ...
        for i in range(0, len(records), batch_size):
            ...
            td_numbers = [
                str(r.get(found_cols["TD NUMBER"], "")).strip()
                for r in batch
                if str(r.get(found_cols["TD NUMBER"], "")).strip()
            ]
            ...
            for index_in_batch, row in enumerate(batch):
                try:
                    ...
                    lot_val = (
                        str(row.get(found_cols["LOT"], "")).strip()
                        if found_cols["LOT"]
                        else ""
                    )
                    blk_val = (
                        str(row.get(found_cols["BLK"], "")).strip()
                        if found_cols["BLK"]
                        else ""
                    )
        ...
        log_action(
            user,
            f"Bulk Assessment Import: {inserted} new, {updated} updated.",
            db_session=db_session,
        )
        db_session.commit()
...
def validate_payment_import(file_content, file_extension, db_session: Session = None):
    ...
    try:
        df = (
            ...
            if file_extension.lower() != ".csv"
            else pd.read_csv(io.BytesIO(file_content))
        )
        df.columns = [str(c).strip().upper() for c in df.columns]
        ...
        for index, row in df.iterrows():
            ...
            or_no = (
                DataCleanser.to_str(row.get(found_cols["or_number"]))
                if found_cols.get("or_number")
                else ""
            )
            amt = (
                DataCleanser.to_float(row.get(found_cols["amount_paid"]))
                if found_cols.get("amount_paid")
                else 0.0
            )
            pnlty = (
                DataCleanser.to_float(row.get(found_cols["penalty"]))
                if found_cols.get("penalty")
                else 0.0
            )
            dscnt = (
                DataCleanser.to_float(row.get(found_cols["discount"]))
                if found_cols.get("discount")
                else 0.0
            )
            excel_av = (
                DataCleanser.to_float(row.get(found_cols["assessed_value"]))
                if found_cols.get("assessed_value")
                else 0.0
            )
            ...
            if not or_no:
                errors.append("Missing OR Number")
            if amt <= 0 and pnlty <= 0:
                errors.append("Invalid Amount")

            # ── Duplicate detection ───────────────────────────────────────
            tax_yr_str = (
                DataCleanser.to_str(row.get(found_cols["tax_year"]))
                ...
                data_to_import.append(
                    {
                        ...
                        "date_paid": (
                            DataCleanser.to_str(row.get(found_cols["date_paid"]))
                            if found_cols.get("date_paid")
                            else ""
                        ),
                        "posted_by": "NONE",
                    }
                ...
                with db_session.begin_nested():
                    ...
                    if not billing:
                        ...
                        if is_prop_year and prop:
                            ...
                            billing_disc = max(
                                billing_disc, float(prop.discount or 0.0)
                            )
                        ...
                        billing.penalty = float(billing.penalty or 0.0) + float(
                            row.get("penalty", 0.0)
                        )
                        billing.discount = float(billing.discount or 0.0) + float(
