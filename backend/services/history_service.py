...
def log_data_change(user_id: int, table_name: str, record_id: int, action: str, 
...
def undo_last_action(user_id: int, db_session: Session = None):
    ...
    if not log.before_value:
        return False, f"Action '{log.action}' on {log.table_name} cannot be undone (no state snapshot)."
