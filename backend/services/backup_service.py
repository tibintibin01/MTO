...
def run_hybrid_backup(user=None):
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INFO: Hybrid Backup Started by {user_name}")
    
    backup_status["is_running"] = True
    try:
        # 1. Local Backup
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INFO: Step 1/5: Creating local SQL dump...")
        ...
        if not success:
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ERROR: Local dump failed.")
            return False, "Local dump failed. Check if mysqldump is installed and configured."
        ...
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INFO: Verification Result: {backup_status['last_verify']}")
        
        # 3. Rotation (Local)
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INFO: Step 3/5: Rotating old local backups...")
        _rotate_backups(LOCAL_DIR, keep=7)
        
        # 4. USB Mirroring
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INFO: Step 4/5: Syncing to USB Drive...")
        ...
        if usb_path:
            ...
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INFO: USB Sync Skipped (No drive found)")
            
        # 5. Cloud Sync
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INFO: Step 5/5: Syncing to Cloud...")
        _sync_to_cloud(local_path)
        backup_status["last_cloud"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] INFO: Hybrid Backup Completed Successfully.")
...
def _create_local_dump(dest_path):
    ...
    try:
        with open(dest_path, 'w', encoding='utf-8') as f:
            ...
            cmd = [dump_path, f"-u{db_user}", f"-h{db_host}", "--single-transaction", db_name]
            if db_pass: cmd.insert(2, f"-p{db_pass}")
            
            subprocess.run(cmd, stdout=f, check=True, timeout=300)
        return True
    except subprocess.TimeoutExpired:
        log_error_to_file("Backup Dump Timeout", "mysqldump took longer than 5 minutes.")
        return False
    except Exception as e:
