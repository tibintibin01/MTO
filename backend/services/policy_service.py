...
def get_retention_summary(db_session: Session = None):
    ...
    prop_count = (
        ...
        .filter(Property.created_at < ten_years_ago, Property.deleted_at == None)
        .scalar()
    )

    pay_count = (
        db_session.query(func.count(Payment.id))
        .filter(Payment.date_paid < ten_years_ago.date())
        .scalar()
    )
...
def run_archival_policy(user="SYSTEM", db_session: Session = None):
    ...
    try:
        ...
        db_session.execute(
            text("CREATE TABLE IF NOT EXISTS properties_archive LIKE properties")
        )
        db_session.execute(
            text("CREATE TABLE IF NOT EXISTS payments_archive LIKE payments")
        )

        # 2. Archive Old Properties
        db_session.execute(
            text(
                "INSERT INTO properties_archive SELECT * FROM properties WHERE created_at < :cutoff"
            ),
            {"cutoff": cutoff_date},
        )
        ...
        db_session.execute(
            text(
                "INSERT INTO payments_archive SELECT * FROM payments WHERE date_paid < :cutoff"
            ),
        ...
        # 4. Log the policy enforcement in the sealed audit chain.
        from backend.services.history_service import log_data_change

        log_data_change(
            user_id=user.get("id") if isinstance(user, dict) else None,
            username=(
                user.get("username", "SYSTEM") if isinstance(user, dict) else str(user)
            ),
            action="RETENTION_POLICY_ENFORCED",
            table_name="retention_policy",
            record_id=None,
            after={"cutoff_date": cutoff_date.date().isoformat()},
