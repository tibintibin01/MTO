...
def global_search(query, limit=10):
    ...
    prop_query = """
        ...
        LIMIT %s
    """
    props = db.db_query(prop_query, (search_term, search_term, search_term, limit), fetch=True, commit=False)
    for p in props or []:
        results.append({
            "id": p[0],
            "title": f"🏠 {p[1]}",
    ...
    pay_query = """
        ...
        LIMIT %s
    """
    pays = db.db_query(pay_query, (search_term, search_term, limit), fetch=True, commit=False)
    for pay in pays or []:
        results.append({
            "id": pay[0],
            "title": f"📄 OR: {pay[1]}",
...
def get_quick_actions():
    ...
    return [
        {"title": "⚡ Create New Property", "command": "nav:new_property", "type": "action"},
        {"title": "⚡ Run System Backup", "command": "action:backup", "type": "action"},
        {"title": "⚡ View Delinquency Report", "command": "nav:reports", "type": "action"},
