...
def get_username(user):
    if isinstance(user, dict):
        return str(user.get("username") or user.get("full_name") or user.get("id") or "System")
...
def acquire_user_lock(user_id, user_name, stale_minutes=30):
    return db._acquire_named_lock("user_edit_locks", "user_id", user_id, user_name, stale_minutes)
...
def verify_user_login(username, password):
    ...
    if stored_password and not is_password_hashed(stored_password):
        ...
        db.db_query("UPDATE users SET password=%s WHERE id=%s", (upgraded_hash, user_id))
...
def create_user(username, full_name, password, role, admin_user):
    ...
    def operation(cur):
        ...
        db.record_audit_log_with_cur(
            cur, admin_user, f"Created new user: {username} ({full_name})",
    ...
    return [
        {
            ...
            "is_active": bool(r[4]), "last_login": r[5], "created_at": r[6]
        }
        for r in rows
    ]
...
def update_user_role(user_id, new_role, admin_user):
    def operation(cur):
        ...
        db.record_audit_log_with_cur(
            cur, admin_user, f"Changed role for user ID {user_id}",
            table_name="users", record_id=user_id,
...
def update_user_status(user_id, is_active, admin_user):
    def operation(cur):
        ...
        cur.execute("UPDATE users SET is_active=%s WHERE id=%s", (int(is_active), user_id))
        
        db.record_audit_log_with_cur(
            cur, admin_user, f"{'Enabled' if is_active else 'Disabled'} user ID {user_id}",
            table_name="users", record_id=user_id,
            old_values={"is_active": old_status}, new_values={"is_active": is_active}
        )
        return True
    return db.execute_transaction(operation)
...
def reset_user_password(user_id, new_password, admin_user):
    def operation(cur):
        ...
        db.record_audit_log(
            admin_user, f"Force password reset for user ID {user_id}",
            table_name="users", record_id=user_id
        )
        return True
    return db.execute_transaction(operation)
